...
const SalesOrder = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        const response = await fetch(`${BACKEND_URL}/api/orders?type=sales`, {
          method: "GET",
          headers: {
  ...
  return (
    <div className="bg-productBG md:px-12 px-4 py-12 min-h-[83vh]">
      ...
      {loading ? (
        ...
        <p className="text-center mt-5 text-gray-600">No sales orders found.</p>
      ) : (
        orders.map((order) => (
            ...
            <div className="md:hidden bg-white py-2.5 px-5 rounded-xl flex flex-col items-center space-y-4">
              ...
              <div className="flex justify-center space-x-4">
                {order.status === "pending" ? (
                  <>
                    <button
                      onClick={() => openModal(order.id, "delivered")}
